...
export default function GeneratePage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      addLog(
        'info',
        `開始生成 / Start generation: ${endpoint}, size=${getImageSize(values)}, quality=${values.quality}, n=${values.n}`,
      )
      ...
      addLog('success', `生成完成 / Generation completed: ${result.images.length} image(s), ${duration}s${usageStr}`)
      ...
      setError(error)
      const duration = Math.round((Date.now() - startedAt) / 1000)
      addLog('error', `生成失敗 / Generation failed: ${error}, ${duration}s`)
    } finally {
      setLoading(false)
    }
  }
